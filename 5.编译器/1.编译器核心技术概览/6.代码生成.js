(() => {
  ...
  /* 生成器相关函数 */
  // 生成节点
  function _generateJsNode(jsNode, context) {
    const { push, indent, deIndent } = context;
    switch (jsNode.type) {
      case "FunctionDecl":
        // 添加函数名
        push(`function ${jsNode.id.name} (`);
        // 添加参数
        _generateJsNodeList(jsNode.params, context);
        // 添加括号
        push(`) {`);
        // 缩进
        indent();
        // 函数体
        jsNode.body.forEach((node) => {
          _generateJsNode(node, context);
        });
        // 减少缩进
        deIndent();
        // 添加括号
      ...
      case "ReturnStatement":
        ...
        push(`return `);
        // 返回内容
        _generateJsNode(jsNode.return, context);
        break;
      case "CallExpression":
        // 调用函数
        push(`${jsNode.callee.name}(`);
        // 函数参数
        _generateJsNodeList(jsNode.arguments, context);
        // 函数结束括号
        push(`)`);
        break;
      case "StringLiteral":
      ...
      case "ArrayExpression":
        ...
        push(`[`);
        // 数组节点
        _generateJsNodeList(jsNode.elements, context);
        // 添加结束括号
        push(`]`);
        break;
    }
  ...
  // 生成节点列表
  function _generateJsNodeList(jsNodes, context) {
    const { push } = context;
    for (let i = 0; i < jsNodes.length; i++) {
      _generateJsNode(jsNodes[i], context);
      // 除了末尾 都加个逗号
      if (i < jsNodes.length - 1) {
        push(`, `);
      }
    }
  }
  ...
  function generate(jsNode) {
    const context = {
      ...
      // 换行
      newLine() {
        context.code += "\n" + "  ".repeat(context.currIndentLevel);
      },
      // 缩进
      indent() {
        context.currIndentLevel++;
        context.newLine();
      },
      // 减少缩进
      deIndent() {
    ...
    _generateJsNode(jsNode, context);
    return context.code;
  }

  // 编译器
  function compile(tempalte) {
    // 解析生成ast
    const ast = parser(tempalte);
    // 生成jsAst
